...
function AppShell({ children, project, displayName = '创作者', view = 'home', onNavigate, onOpenCreator, onOpenWritingRoom, onOpenAssistant, onOpenSearch, onOpenNotifications, onOpenProfile }) {
  const progress = progressFor(project)
  const currentTitle = project?.title || '还没有开始作品'
  const avatar = String(displayName || '创').trim().slice(0, 1) || '创'
  return <div className="nf-app-shell">
    <aside className="nf-sidebar">
      ...
      <footer className="nf-user-row"><button type="button" className="nf-user-trigger" onClick={onOpenProfile}><span className="nf-avatar">{avatar}</span><strong>{displayName}</strong><ChevronDown size={15} /></button><button type="button" title="切换显示模式" onClick={() => onNavigate?.('settings')}><Menu size={16} /></button></footer>
    </aside>
