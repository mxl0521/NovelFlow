...
const writtenCount = (project) => project?.chapters?.filter((chapter) => String(chapter.body || '').trim()).length || 0
...
function HomePage({ project, projects, displayName = '创作者', loading, error, onRetry, onCreate, onOpenProject, onOpenWritingRoom, onProjectsChanged }) {
  ...
  return <div className="nf-home">
    <section className="nf-home-intro"><div><h1>下午好，{displayName}</h1><p>今天可以从一句话开始，阿流会帮你把它变成可写的方向。</p></div><blockquote>“故事不会凭空出现，<br />只有你能让它发生。”</blockquote></section>
    ...
    <section className="nf-overview" aria-labelledby="nf-overview-title"><h2 id="nf-overview-title">写作概览</h2><div className="nf-stat-grid"><article><span>创作中</span><strong>{projects.length}</strong><small>个故事</small></article><article><span>已写章节</span><strong>{writtenCount(project)}</strong><small>章</small></article><article><span>总字数</span><strong>{wordCount(project).toLocaleString('zh-CN')}</strong><small>字</small></article><article><span>创作天数</span><strong>{project ? 1 : 0}</strong><small>天</small></article></div></section>
